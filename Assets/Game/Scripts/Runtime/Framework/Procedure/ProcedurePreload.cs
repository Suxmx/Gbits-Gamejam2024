    ...
    public class ProcedurePreload : ProcedureBase
    {
        ...
        protected override void OnUpdate(ProcedureOwner procedureOwner, float elapseSeconds, float realElapseSeconds)
        {
            ...
            foreach (KeyValuePair<string, bool> loadedFlag in m_LoadedFlag)
            {
                if (!loadedFlag.Value)
                {
                    Debug.Log($"{loadedFlag.Key} {loadedFlag.Value}");
                    return;
                }
            }
            ...
            else if (!string.IsNullOrEmpty(GameEntry.Procedure.TestSceneName)) //直接进入测试场景
            {
                procedureOwner.SetData<VarString>("NextScene", GameEntry.Procedure.TestSceneName);
            }
            else //进入Main流程场景
            {
