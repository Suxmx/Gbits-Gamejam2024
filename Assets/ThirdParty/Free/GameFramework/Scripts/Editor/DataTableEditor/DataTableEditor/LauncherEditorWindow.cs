...
namespace DataTableEditor
{
    public class LauncherEditorWindow : EditorWindow
    {
        ...
        {
            m_EncodingSelectionIndex =
                EditorPrefs.GetInt("DataTableEditor_" + Application.productName + "_EncodingSelectionIndex", 0);
        ...
        private void OnGUI()
        {
            ...
            m_EncodingSelectionIndex =
                EditorGUILayout.Popup("数据表编码", m_EncodingSelectionIndex, m_EncodingSelectionStrings);
        ...
        private void EncodingCheck()
        {
            ...
            {
                m_CurrentEncoding = Encoding.GetEncoding(m_AllEncodingTypeList[m_EncodingSelectionIndex].Name);
                EditorPrefs.SetInt("DataTableEditor_" + Application.productName + "_EncodingSelectionIndex",
                    m_EncodingSelectionIndex);
        ...
        {
            var m_DataTableEditingWindow = new DataTableEditingWindowInstance();
            var path = EditorPrefs.GetString($"{Application.productName}_DataTablePath", Application.dataPath);
            string filePath = EditorUtility.OpenFilePanel("加载数据表格文件", path, "txt");
            if (!string.IsNullOrEmpty(filePath))
            {
                EditorPrefs.SetString($"{Application.productName}_DataTablePath", Path.GetDirectoryName(filePath));
