...
namespace UnityGameFramework.Editor
{
    ...
    internal sealed class ProcedureComponentInspector : GameFrameworkInspector
    {
        ...
        private List<string> m_AvailableSceneNames;
        private int m_TestSceneIndex;
        private string m_TestSceneName;
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUI.BeginDisabledGroup(EditorApplication.isPlayingOrWillChangePlaymode);
            {
                ...
                //DEBUG
                EditorGUILayout.Separator();
                GUILayout.Label("Debug 设置(FOR EDITTOR ONLY)", EditorStyles.boldLabel);
                {
                    bool selected = EditorGUILayout.ToggleLeft("是否进入菜单",  m_IfEnterMenuProp.boolValue);
                    if ( m_IfEnterMenuProp.boolValue != selected)
                    {
                ...
                if (m_AvailableSceneNames.Count > 0)
                {
                    int selectedIndex = EditorGUILayout.Popup("ProcedureMain对应的Scene", m_TestSceneIndex,
                        m_AvailableSceneNames.ToArray());
                    if (selectedIndex != m_TestSceneIndex)
                    {
                        m_TestSceneIndex = selectedIndex;
                        if (selectedIndex == 0) m_TestSceneName = "";
                        else m_TestSceneName = m_AvailableSceneNames[m_TestSceneIndex];
                        m_TestSceneNameProp.stringValue = m_TestSceneName;
                    }
                }
                else
                {
                    EditorGUILayout.HelpBox($"请先在{s_SearchScenePaths[0]}中放置场景", MessageType.Info);
                }
        ...
        private void FindAllScenes()
        {
            ...
            m_AvailableSceneNames = new() { "正常进入" };

            foreach (string sceneGuid in sceneGuids)
            {
                string sceneName = AssetDatabase.GUIDToAssetPath(sceneGuid);
                sceneName = Path.GetFileName(sceneName).Split(".")[0];
                if (sceneName == "Launcher") continue;
                m_AvailableSceneNames.Add(sceneName);
            }

            if (string.IsNullOrEmpty(m_TestSceneNameProp.stringValue))
            {
                m_TestSceneIndex = 0;
                return;
            }

            m_TestSceneIndex = m_AvailableSceneNames.FindIndex(x => x == m_TestSceneNameProp.stringValue);
        }
